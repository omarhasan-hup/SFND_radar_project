...
d_0=100; % intail distance in meters
v_0=55;  %intial velocity in m/s
F=77e9;
Max_range=200;
R_res=1;
V_max=100;
c=3e8;
B=c/(2*R_res) %bandwidth
Tchirp=(5.5*2*Max_range)/c  %Tchrip
S=B/Tchirp                  %slope
...
for i=1:length(t)         
    ...
    %For each time stamp update the Range of the Target for constant velocity. 
     r_t(i) =d_0+ v_0*t(i);  % to update the time each iteration
     td(i) = 2*r_t(i)/c;
    % *%TODO* :
    %For each time sample we need update the transmitted and
    %received signal. 
    Tx(i) =cos(2*pi*(fc*t(i)+((S*(t(i)^2))/2))); %transmitted signal 
    
    Rx (i)  =cos(2*pi*(fc*(t(i)-td(i))+((S*((t(i)-td(i))^2))/2)));
    %the receved signal after the assumed time delay vector
...
% those three equations as shown in the first video in the project walk throw
...
for j=1:Nd-2*(Tcr+Gcr)
    for i=1:Nr/2-2*(Tcd+Gcd)
        %I want to extract only the Training cells. To do that, I will get
        %the sliding patch and, after converting it to power from decibel,
        %I will set to zero whatever it is not in the position of
        ...
        sigThresh = noise_level(i,j)*offest;
